...
function renderPhotos(){
    for(i=0; i<PHOTONUM; i++){
        document.body.innerHTML += "<img src='../../Shows/img"+i+".jpg' style='height: 50vw; z-index: 1; padding: 1%' onclick='selectPhoto("+i+")' id='img"+i+"'>"
    }
}
function selectPhoto(id){
    ...
    document.getElementById("img"+selectedPhoto).style.width = oldDimensions.w + "px"
    setTimeout(Scale,1, window.innerWidth*0.9+"px", oldDimensions.h*((window.innerWidth*0.9)/oldDimensions.w) + "px")
    setTimeout(move, 1, id, "5%", String((window.innerHeight/2) - (oldDimensions.h*((window.innerWidth*0.9)/oldDimensions.w))/2) + "px")
}

function Scale(x, y){
    document.getElementById("img"+selectedPhoto).style.width = x
    document.getElementById("img"+selectedPhoto).style.height = y
}
...
function deselect(){
    document.getElementById("img"+selectedPhoto).style.height = oldDimensions.h + "px"
    document.getElementById("img"+selectedPhoto).style.width = oldDimensions.w + "px"
...
function changeImage(direction){
    ...
    console.log(displayedPhoto)
    document.getElementById("img"+selectedPhoto).src = "../../Shows/img"+displayedPhoto+".jpg"
